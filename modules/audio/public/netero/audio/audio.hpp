...
enum class RtCode {
    ...
    ABILITY_NOT_SUPPORTED =
        1, /**< Functionality not supported. Issued while the native device does not support the call. */
    FORMAT_NOT_SUPPORTED = 2, /**< The native device does not support the requested format. */
    DEVICE_TIMEOUT =
        3, /**< Time out while accessing/getting device's resource. The device is probably busy, try later. */
    ERR_NATIVE =
        4, /**< Native error. Issued while native call failed, usually the last error msg is set or an deviceError signal is issued. */
    ERR_MISSING_CALLBACK =
        5, /**< Audio callback is missing. You forgot to setup the audio callback. */
    ...
    ERR_ALTER_RUNNING =
        8, /**< You try to change a device or a handler while the device is running. Stop the device before. */
...
struct StreamFormat {
    ...
    unsigned framesCount =
        0; /**< Number of frames in contained in the shared buffer with the device. */
    ...
    std::vector<float>
        supportedSamplingRate = {}; /**< Supported sampling frequency of the native audio device. */
};
