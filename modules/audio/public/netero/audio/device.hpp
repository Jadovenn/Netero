...
struct device {
    ...
    std::string
         manufacturer; /**< Device manufacturer name. May be "Unknown" if not provided by the audio backend. */
    bool isLoopback =
        false; /**< This is a loopback device. This flag is set while OS support loopback device and it is actually one of them. */
    ...
    struct signalsSet {
        ...
        netero::signal<void(float *, const size_t)> *renderStreamSig =
            nullptr; /**< Signal emitted while device's buffer available for rendering. */
        netero::signal<void(const float *, const size_t)> *captureStreamSig =
            nullptr; /**< Signal emitted while device's buffer available for capture. */
        netero::signal<void(const StreamFormat &)> *deviceStreamFormatChangeSig =
            nullptr; /**< Signal emitted while the configuration format changed. */
        netero::signal<void(const std::string &)> *deviceErrorSig =
            nullptr; /**< Signal emitted while the device encounter an error. */
        netero::signal<void(const std::string &)> *deviceDisconnectedSig =
            nullptr; /**< Signal emitted just before the device become unavailable. */
    };
