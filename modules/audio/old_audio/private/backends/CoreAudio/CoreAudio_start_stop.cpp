                                                      ...
                                                      void *infoPointer)
{
    ...
    if (device->deviceId == inDevice) {
        if (device->buffer.callBack) {
            auto buffer =
                reinterpret_cast<float *>(outOutputData->mBuffers[device->buffer.id].mData);
            size_t frames = outOutputData->mBuffers[device->buffer.id].mDataByteSize /
                device->format.bytesPerFrame;
...
netero::audio::RtCode netero::audio::engine::impl::start()
{
    ...
    if (result != noErr || dataSize == 0) {
        std::string msg("netero_audio: Error(" + std::to_string(static_cast<unsigned>(result)) +
                        ") could not retrieve stream configuration for device " +
                        std::to_string(_device.deviceId));
        return RtCode::ERR_NATIVE;
    }
    ...
    result = AudioObjectSetPropertyData(id, &property, 0, nullptr, dataSize, &buffer_frame_size);
    ...
    if (nominalRate !=
        _device.format.samplingFrequency) { // if device sampling rate different, reconfigure it
        ...
        AudioObjectPropertyAddress cb_properties = { kAudioDevicePropertyNominalSampleRate,
                                                     kAudioObjectPropertyScopeGlobal,
                                                     kAudioObjectPropertyElementMaster };
        result = AudioObjectAddPropertyListener(id,
                                                &cb_properties,
                                                rateListener,
        ...
        if (result != noErr) { // Error could not set new sampling rate
            AudioObjectRemovePropertyListener(id,
                                              &cb_properties,
                                              rateListener,
                                              reinterpret_cast<void *>(&reportedRate));
            return RtCode::ERR_NATIVE;
    ...
    if (description.mFormatID != kAudioFormatLinearPCM || description.mBitsPerChannel < 16) {
        ...
        for (auto &format : physicalFormats) {
            ...
            result =
                AudioObjectSetPropertyData(id, &property, 0, nullptr, dataSize, &testDescription);
            if (result == noErr) {
    ...
    if (AudioObjectHasProperty(id, &property)) {
        ...
        if (result == kAudioHardwareNoError) {
            (void)
                latency; // Idk what to do with this, for sure this is not normal so I let it here until I find out.
        }
        else {
    ...
    result = AudioDeviceCreateIOProcID(id,
                                       callbackHandler,
