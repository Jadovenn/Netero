...
netero::audio::WaveFormat netero::audio::engine::impl::getFormat()
{
    ...
    if (result != noErr || size == 0) {
        delete[] bufferList;
        std::string msg("netero_audio: Error(" + std::to_string(static_cast<unsigned>(result)) +
                        ") while getting stream configuration for device " +
    ...
    if (result != kAudioHardwareNoError || size == 0) {
        std::string msg("netero_audio: Error(" + std::to_string(static_cast<unsigned>(result)) +
                        ") while getting sampling rate on device " +
                        std::to_string(_device.deviceId));
        return format;
    }
    ...
    format.supportedSamplingRate.erase(
        std::unique(format.supportedSamplingRate.begin(), format.supportedSamplingRate.end()),
        format.supportedSamplingRate.end());
    if (format.supportedSamplingRate.empty()) {
        std::string msg("netero_audio: Error no supported sampling rate found on device " +
                        std::to_string(_device.deviceId));
        return format;
    }
    return format;
