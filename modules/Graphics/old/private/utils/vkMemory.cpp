...
std::pair<VkBuffer, VkDeviceMemory> AllocBuffer(netero::graphics::Device* device,
                                                VkDeviceSize              size,
                                                ...
                                                VkMemoryPropertyFlags     properties)
{
    ...
    allocInfo.memoryTypeIndex =
        FindMemoryType(device->physicalDevice, memRequirements.memoryTypeBits, properties);

    if (vkAllocateMemory(device->logicalDevice, &allocInfo, nullptr, &memory.second) !=
        VK_SUCCESS) {
...
std::pair<VkImage, VkDeviceMemory> AllocImage(netero::graphics::Device* device,
                                              uint32_t                  width,
                                              uint32_t                  height,
                                              VkFormat                  format,
