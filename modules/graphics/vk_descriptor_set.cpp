...
DescriptorSets::DescriptorSets(Device* device, uint32_t descriptorSetCount)
    : _device(device), _descriptorCount(descriptorSetCount)
{
...
void DescriptorSets::_createDescriptorSetPool()
{
    ...
    if (vkCreateDescriptorPool(this->_device->logicalDevice,
                               &poolInfo,
                               nullptr,
...
void DescriptorSets::_createDescriptorSets()
{
    ...
    if (vkAllocateDescriptorSets(this->_device->logicalDevice,
                                 &allocInfo,
                                 this->_descriptorSets.data()) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate descriptor sets!");
    }
}

DescriptorSets::Error
DescriptorSets::write(Descriptor& descriptor, int setIdx, VkImageView imageView, VkSampler sampler)
{
    ...
    if (setIdx != WHOLE_SIZE && setIdx < 0 &&
        setIdx >= static_cast<int>(this->_descriptorSets.size())) {
...
DescriptorSets::Error DescriptorSets::write(Descriptor&  descriptor,
                                            int          setIdx,
                                            VkBuffer     buffer,
                                            VkDeviceSize offset,
                                            VkDeviceSize range)
...
DescriptorSets::Error DescriptorSets::_write(Descriptor&             descriptor,
                                             int                     setIdx,
                                             VkDescriptorBufferInfo* bufferInfo,
                                             VkDescriptorImageInfo*  imageInfo)
{
