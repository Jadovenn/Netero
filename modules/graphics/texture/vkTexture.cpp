...
void Texture::transferTextureToGPU()
{
    VkCommandBuffer transferCmd =
        vkUtils::BeginCommandBufferRecording(this->_device->logicalDevice,
                                             this->_device->transferCommandPool,
                                             VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);

    auto [image, imageMemory] =
        vkUtils::AllocImage(this->_device,
                            this->_width,
                            this->_height,
...
void Texture::createTexturesSampler()
{
    ...
    if (vkCreateSampler(this->_device->logicalDevice,
                        &samplerInfo,
                        nullptr,
...
void Texture::loadTexture(const std::string& path, TextureSamplingMode samplingMode)
{
    ...
    void* data;
    vkMapMemory(this->_device->logicalDevice, stagingBufferMemory, 0, imageSize, 0, &data);
