...
class Context {
    ...
    std::vector<VkSemaphore>
                             _imageAvailableSemaphore; /**< Semaphore synchronizing available image to be rendered. */
    std::vector<VkSemaphore> _renderFinishedSemaphore; /**< Semaphore synchronizing end of frame. */
    std::vector<VkFence>
        _inFlightFences; /**< Fence to synchronise fences in flight. Synchronise GPU/CPU. */
    std::vector<VkFence>
              _imagesInFlight; /**< Fence to synchronise frame in flight vs it is still on processing from GPU. Synchronise GPU/CPU. */
    size_t    _currentFrame = 0; /**< The current index of the frame in processing. */
    const int MAX_FRAMES_IN_FLIGHT =
        2; /**< The maximum number of frame to be processed at the same time. */
